...
class SpaceSaving():
    ...
    def swap(self, tuple_x, tuple_y):
        assert len(tuple_x) == 2
        assert len(tuple_y) == 2

        x, x_val = tuple_x
        y, y_val = tuple_y      
        
        x_weight_index = self.item_to_indices[x][0]
        y_weight_index = self.item_to_indices[y][0]
    ...
    def insertUnmonitored(self, item):
    ...
    def update(self, x, val=1):
        ...
        if x in self.item_to_indices:
            self.updateMonitored(x)
        else:
            if self.size < self.k:
                assert(val > 0)
                self.size += 1
                self.insertUnmonitored(x)
            else:
                if val > 0:
                    # replace min
                    ...
                    self.updateMonitored(min_item)
                    weight_index = self.item_to_indices[min_item][0]
