...
class Panakos():
    ...
    def cardinality(self, offline = True):
        heavyCard = 0
        bitmap_copy = None
        if offline:
            ...
            if not heavyFlows:
                ...
                for freq, items in heavyFlows.items():
                    for item in items[1:]:
                        heavyCard += 1
                        bitmap_copy[self._hash(item) % self.bitmapLen] = 0
        else:
            bitmap_copy = self.bitmap.copy()
        dic = collections.Counter(bitmap_copy)
        return heavyCard + self.bitmapLen * math.log(1.0 * self.bitmapLen / dic[0])
    ...
    def pmf(self):
        ...
        minFreq, maxFreq = float('inf'), 0
        dic_arrays = [None for i in range(self.countMinRows)]
        temp = 0
        for table in CountMin_copy.tables:
            list_table = list(table)
            minFreq = min(minFreq, min(list_table))
            maxFreq = max(maxFreq, max(list_table))
            dic_arrays[temp] = collections.Counter(list_table)
            temp += 1
        ...
        for freq in range(minFreq, maxFreq+1):
            ...
            if freq  == minFreq:
                ...
                dic_freq_arr = [dic[freq] for dic in dic_arrays]
                if len(dic_freq_arr) > 0:
